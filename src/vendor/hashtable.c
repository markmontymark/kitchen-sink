...
*/
int hash_table_add(hash_table_t * table, void * key, size_t key_len, void * val, size_t val_len)
{
    ...
    if (table->mode == MODE_COPY)
    {
        ...
        element->val = malloc(val_len);
        if (element->key && element->val)
        {
            memcpy(element->key, key, key_len);
            memcpy(element->val, val, val_len);
        }
        else
        {
            if (element->key)
...
int hash_table_iterate(hash_table_t * table, void(*func_per_element)(void *,void *))
{
    size_t i = 0;
    size_t count = 0;
    //keys = calloc(table->key_count, sizeof(void *));
    for(i=0;i<HASH_LEN;i++)
    {
        if (table->store_house[i])
        {
            //keys[count++] = table->store_house[i];
            hash_table_element_t *temp = table->store_house[i];
				( * func_per_element )((char *)temp->key,temp->val);
            while(temp->next)
            {
                //keys[count++] = temp->next;
                temp = temp->next;
					( * func_per_element )( temp->key, temp->val );
            }
        }
    }
}
int hash_table_iterate_file(hash_table_t * table, FILE * fp, void(*func_per_element)(FILE *,void *,void *))
{
    ...
    for(i=0;i<HASH_LEN;i++)
    {
        if (table->store_house[i])
        {
            //keys[count++] = table->store_house[i];
            hash_table_element_t *temp = table->store_house[i];
				( * func_per_element )(fp, (char *)temp->key,temp->val);
            while(temp->next)
            {
                //keys[count++] = temp->next;
                temp = temp->next;
					( * func_per_element )( fp, temp->key, temp->val );
            }
        }
    }
}
...
{
	hash_table_iterate_file(table,fp,hash_table_dump_func);
}
